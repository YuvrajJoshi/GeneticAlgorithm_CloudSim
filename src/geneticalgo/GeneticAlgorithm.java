...
public class GeneticAlgorithm {
	...
	private static List<Vm> createVM(int userId, int vms) {
		...
		int mips = 3500;		// Million Instructions Per Sec
		...
		for (int i = 0; i < vms; i++) {
			...
			vm[i] = new Vm(i, userId, mips + rOb.nextInt(3500), 
						pesNumber, ram, 
						bw, size, vmm, 
	...
	private static List<Cloudlet> createCloudlet(int userId, int cloudlets) {
		...
		// Cloudlet parameters
		long length = 5000;			// million instructions(MI)
	...
	public static void main(String[] args) {
		...
		try {
			...
			time = parameters[2];
			double[] totalTime = new double[2*numVms];		// for showing comparison of most fit times of every generation 
			/*
			 * START PRODUCING GENERATIONS 
			 * run for [2*numVms] number of generations
			 */
			for(int itr = 0; itr < 2*numVms; itr++){				
				...
				if(mutProb < 0.5)
					gf.mutation(r, initialPopulation, populationSize, 
							sortedVmList, numCloudlets, numVms);
			...
			printCloudletList(result, totalTime);
	 ...
	 */
	private static void printCloudletList(List<Cloudlet> list, double[] totalTime) {
		int size = list.size();
		Cloudlet cloudlet;
		...
		Log.printLine("\nCumulative Least Time of tasks of all the Generations produced: \n");
		Log.printLine("Generation" + indent + "Fittest Least Time");
		
		for (int itr = 0; itr < totalTime.length; itr++) 
			Log.printLine(indent + itr + "-th" + indent + indent + totalTime[itr]);
