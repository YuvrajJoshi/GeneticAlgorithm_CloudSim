...
public class GeneticFunctions {
	...
	/**
	 * Mutates a random Chromosome by replacing a random VMs by the VM with most mips.
	 * 
	...
	public void mutation(Random rand, ArrayList<Chromosomes> initialPopulation, int populationSize,
									ArrayList<Vm> sortedVmList, int numCloudlets, int numVms) {
		...
		// Update
		int swap_index;
		swap_index = rand.nextInt((numCloudlets / numVms) - 1);	// generate a random point
		
		// Update		
		mutchromosome.updateGene(swap_index,  highestMipsVM);
